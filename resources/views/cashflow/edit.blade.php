        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100 flex flex-col gap-8">
                    ...
                    <form action="{{ route('cashflow.update', [$project->id, $cashflow->id]) }}" method="post">
                        @csrf
                        @method('put')
                        ...
                        <div class="flex gap-4">
                            <div class="flex-1 mb-4">
                                ...
                                <x-text-input min=0 id="year" class="block mt-1 w-full" type="number" name="year" :value="old('year', $cashflow->year)" />
                                <x-input-error :messages="$errors->get('year')" class="mt-2" />
                            </div>

                            <div class="flex-1 mb-4">
                                <x-input-label for="production" :value="__('Production (Mbbl)')" />
                                <x-text-input min=0 id="production" class="block mt-1 w-full" type="number" name="production" :value="old('production', $cashflow->production)" />
                                <x-input-error :messages="$errors->get('production')" class="mt-2" />
                            </div>

                            <div class="flex-1 mb-4">
                                <x-input-label for="income" :value="__('Income')" />
                                <x-text-input min=0 id="income" class="block mt-1 w-full" type="number" name="income" :value="old('income', $cashflow->income)" readonly/>
                                <x-input-error :messages="$errors->get('income')" class="mt-2" />
                            </div>

                            <div class="flex-1 mb-4">
                                <x-input-label for="opex" :value="__('Operational')" />
                                <x-text-input min=0 id="opex" class="block mt-1 w-full" type="number" name="opex" :value="old('opex', $cashflow->opex)" />
                            ...
                            <div class="flex-1 mb-4">
                                ...
                                <x-text-input min=0 id="taxable_income" class="block mt-1 w-full" type="number" name="taxable_income" :value="old('taxable_income', $cashflow->taxable_income)" readonly/>
                                <x-input-error :messages="$errors->get('taxable_income')" class="mt-2" />
                            </div>

                            <div class="flex-1 mb-4">
                                <x-input-label for="tax" :value="__('Tax')" />
                                <x-text-input min=0 id="tax" class="block mt-1 w-full" type="number" :value="old('tax', $cashflow->taxable_income*$project->tax/100)" readonly/>
                                <x-input-error :messages="$errors->get('tax')" class="mt-2" />
                            </div>
                            
                            <div class="flex-1 mb-4">
                                <x-input-label for="net_cashflow" :value="__('NCF')" />
                                <x-text-input min=0 id="net_cashflow" class="block mt-1 w-full" type="number" name="net_cashflow" :value="old('net_cashflow', $cashflow->net_cashflow)" readonly/>
